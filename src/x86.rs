...
pub fn read_io_port_u8(port: u16) -> u8 {
    let mut data: u8;
    unsafe {
        asm!("in al, dx",        // IN命令: DXレジスタのポート番号からALレジスタにデータを読み込み
            out("al") data,      // ALレジスタの値をdata変数に出力
            in("dx") port)       // port変数の値をDXレジスタに入力
    }
    data
}
...
pub fn write_io_port_u8(port: u16, data: u8) {
    unsafe {
        asm!("out dx, al",       // OUT命令: ALレジスタの値をDXレジスタのポート番号に書き込み
            in("al") data,       // data変数の値をALレジスタに入力
            in("dx") port)       // port変数の値をDXレジスタに入力
    }
}
